...
def main():
    ...
    # Input/Output
    source = "MM1.mp4"  # Path to input video file or webcam index (0 for default camera)
    output_path = "output.mp4"  # Path to output video file
    
    # Model settings
    yolo_model_size = "medium"  # YOLOv11 model size: "nano", "small", "medium", "large", "extra"
    ...
    while True:
        ...
        try:
            ...
            for detection in detections:
                ...
                class_name = detector.get_class_names()[class_id].lower()

                # — 1) get a reliable depth at the bottom‐center of the license plate —
                x1, y1, x2, y2 = bbox
                bottom_cx = int((x1 + x2) / 2)
                bottom_cy = int(y2)
                depth_plate = depth_estimator.get_depth_at_point(depth_map, bottom_cx, bottom_cy)

                # — 2) estimate the full 3D bounding box —
                box3d = bbox3d_estimator.estimate_3d_box(
                    bbox_2d=bbox,
                    depth_value=depth_plate,
                    class_name=class_name,
                    object_id=obj_id
                )

                # — 3) pull out its real dimensions —
                h, w, l = box3d['dimensions']
                print(f"[3D dims]  h={h:.2f} m, w={w:.2f} m, l={l:.2f} m")

                # — 4) compute the actual 3D point where the license plate hits the ground —
                #    back-project the bottom center point into camera coords:
                world_pt = bbox3d_estimator._backproject_point(
                    bottom_cx,
                    bottom_cy,
                    depth_plate
                )
                print(f"[ground pt] X={world_pt[0]:.2f} m, Y={world_pt[1]:.2f} m, Z={world_pt[2]:.2f} m")
                X,Y,Z = world_pt
                box3d['ground_point'] = world_pt
                # store the enriched 3D box for drawing/tracking
